...
<WeightPopoverCard id="logits" title={'로짓(Logits)'} bind:isAnimationActive {timeline} bind:isOpen>
	<div class="weight-popover-content flex items-center justify-start">
		<div class="matrix flex flex-col items-center">
			<div class="title flex items-center gap-1">
				<span>출력<br />임베딩</span>
				<HelpPopover id="hidden-states" 
					>{`모든 블록을 통과한 뒤, 마지막 토큰의 임베딩 벡터에는 앞선 모든 토큰의 문맥 정보가 담겨 있습니다.`}</HelpPopover
				>
			</div>
			<Matrix
		...
		<div class="matrix flex flex-col items-center">
			<div class="title flex items-center gap-1">
				...
				<HelpPopover id="logits" 
					>{`소프트맥스를 적용하기 전, 어휘의 각 토큰에 대한 모델의 선호도를 나타내는 원시 점수입니다.`}</HelpPopover
				>
			</div>
			<div class="flex">
				<Matrix
